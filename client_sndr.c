...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr,"Usage:  %s <Server Port> <Broadcast port>\n", argv[0]);
        exit(1);
    }
    serverPort = atoi(argv[1]);
    broadcastPort = atoi(argv[2]);/* broadcast port */   
    ...
    //Сообщаем серверу, что этот клиент - отправитель
    ...
    int length;
	srand(time(NULL) - (broadcastPort-60000)*5000);
